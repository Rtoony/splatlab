...
def main() -> None:
    ...
    print(f"[render-views] {means.shape[0]} gaussians, {split} split {n} cams, rendering {ids}", flush=True)
    ...
    def render_rgb(c2w, i, w, h):
        out, alpha, _ = rasterization(
            ...
            rasterize_mode="antialiased", backgrounds=torch.ones(1, 3, device=DEV))
